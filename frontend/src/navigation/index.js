...
import data from './AppetizerData.js';
import datas from './SaladData.js';
import axios from "axios";

import { useAuth } from "../context/AuthContext";
function Navigation() {
    const { saladItems } = datas;
    const { appetizerItems } = data;
    ...
    const checkoutHandler = async (amount) => {
        ...
        const { data: { key } } = await axios.get("https://veganhubserver.vercel.app/api/getkey")

        const { data: { order } } = await axios.post("https://veganhubserver.vercel.app/checkout", {
            amount
        })
        ...
        const options = {
            ...
            order_id: order.id,
            callback_url: "https://veganhubserver.vercel.app/api/paymentverification",
            prefill: {
                name: "shiwang pandey",
